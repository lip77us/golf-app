...
|---|---|---|
| `cup-hub` ✅ **landed (core)** | Six equal-weight rows → **one adaptive primary action** + two quiet nav buttons + a collapsed round-setup disclosure; **Delete → ⋮ menu**; date once | `_TournamentCard` cup branch rebuilt (non-cup path untouched): header shows **course · date once** + a **⋮ menu** carrying Delete (no longer the loudest pixel); a status line ("Not started" / "Round N in progress" / "Cup complete"); **one primary action** that adapts — Enter Scores (live) / Set Up Round N (pending, TD) / View Result (complete) via `_cupHub`; **Leaderboard + Draft & Teams** as two outlined nav buttons (Leaderboard+Scoreboard merged → the `__bandon_cup__` tab via `_openCupTab`); the three round-scoped actions (+ Stroke Play config) collapse into a **"Round setup" `ExpansionTile`** (TD, not complete; Recalculate hidden for pending rounds). **Verified on device** incl. the disclosure + ⋮→Delete confirm. **Deferred:** the live team **tally + clinch bar** on the card (needs a per-card standings fetch — the same data plumbing deferred on the leaderboard); Q3 (non-TD read-only summary) — today a non-TD simply sees no setup disclosure |
| `cup-round-hub` ✅ **landed (core)** | Team **chips** replace tinted names; aligned Tee/CH columns; **Complete Round is TD-only** (the pull-forward behaviour gap) | **Complete Round gating (backend + UI):** `RoundCompleteView` was an open `get_object_or_404` (any authenticated user could close any round) → now `round_for_scorer` (strangers 404) + a **cup rounds are owner-only** check (a scorer gets 403; casual scorer-can-finish preserved). UI hides the button for a cup non-manager, showing a "the TD completes this round" note. Tests: `api/test_delegated_scoring.py::CompleteRoundGatingTests` (stranger 404, owner 200, casual scorer 200, cup scorer 403 / owner 200). **Card:** team **chip** (`cupTeamColor` box + initial) replaces the name-tint (the red-on-white / blue-on-white colour-blind failure), names return to normal ink; **aligned Tee/CH columns** (fixed-width, `_titleCaseTee` normalises "RED"→"Red"); "My Group"→**YOUR GROUP** badge + mint border. Role-scoping already worked (TD `canManage` → Enter Scores on every group; player → own only). Verified on device. **Deferred:** the collapsed header + **live cup tally** (needs a standings fetch into the hub) and a read-only **View card** for a player looking at another in-progress group (needs a verified read-only route) |
| `cup-leaderboard` ✅ **landed (points bar + segment cells)** | **Spatial points bar** (clinch line) + **segment cells** replacing the comma-prose run | **Points bar:** shared `_CupPointsBar` in `_BandonCupScoreboard` + an aggregate clinch header on `_TripleCupOverviewView`. **Segment cells:** `_TripleCupLiveRows` no longer folds the four segments into a collapsed comma-header ("1 UP thru 3, AS thru 3, …") — they're **always-visible labelled cells**. `_TripleCupSubMatchRow` gained a **"· Final"** stamp on a resolved segment (so "1 UP" doesn't read the same banked vs live) and **"Starts N"** for a not-yet-begun segment (Fourball 1 / Foursomes 7 / Singles 13 via `_segmentStartHole`, detected by start-hole > the round's thru, since the backend reports round-progress holes on every sub-match). **Verified on device:** Fourball "Reds · Final", Foursomes "Starts 7", Singles "Starts 13", thru in the header, points bar filled. **Nav restructure NOT done, by decision:** a tournament has exactly one cup (`TeamTournament` is OneToOne with `Tournament`), so a competition **selector** solves a problem that doesn't exist yet, and the fixed-4-view `TabController` rework is high-risk in this 10K-line file for no current payoff — revisit only if multi-cup-per-tournament ever ships |
