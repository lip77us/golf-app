...
| 3 | `cup-handicap` | Mode only (Net / Gross / SO-Low); allowance **per segment** at match-play standard; **strike the net double-bogey cap** (match play has no total to protect) | the details step is now the handicap-only `_StepHandicap`; `NetDoubleBogeyCard` still shown for cups (the bug to fix); dedupe the repeated "Handicap" label; keep `HandicapModeSelector` reuse |
