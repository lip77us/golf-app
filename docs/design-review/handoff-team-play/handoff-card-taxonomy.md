...
- `screens/set-tees.html` — **Set tees** · SetTeesScreen — per-golfer tee picker grouped by sex, Set all per group, tee menu with yards/rating/slope/par
...
- `screens/rabbit-setup.html` — **Rabbit (setup)** · RabbitSetupScreen — mode, match format, halved rabbit, stroke allocation, extra rabbit, stake
- `patterns/sixes-segment-draw.html` — **Sixes — segment draw** · _SegmentDrawSheet — slot-reel draw for the random Segment 2 pairing (fires once, between segments 1 and 2)
...
- `screens/home-casual-rounds.html` — **Rounds (home)** · RoundsListScreen — Active/Completed list, live Active + Completed lists, watched-round filter, 3-row round card
