# Spec: Foursome Play — four-man teams, one round, one leaderboard
...
wanting a bet among its own men still has the casual games.
...
`6 teams — five of four, one of three`. He is never asked to divide before he has a
headcount.
...
What the build screen uniquely offered — **the allowance per team, moving as men move, and
...
themselves from the round hub. A TD inventing six names for men who have not turned up yet
is work nobody asked for, and a colour he never chose is one more thing on screen that
does not help him.
...
**Shamble** — best drive, then each man plays his own ball in. Four scores per hole.
...
the count changes — that is the point of handicapping a three-man team as four.
...
1. **A quota shows its slack.** `4 required / nine · 9 holes · 5 free / nine`. That figure
   tells a captain whether he can let his long hitter drive the par 5.
...
3. **Per nine is two windows.** The front does not carry to the back. A tracker that only
   totals eighteen says a man is fine until the 18th green.
...
7. **Alternating pairs are the team's, set on the 1st tee** — a card before the first score
   ...
   written). Not derived from handicap: four men decide in ten seconds and would override a
   computed pairing anyway. A rota that can be re-cut mid-round is not a rota.
8. **Three men run AB → BC → AC, repeating.** Two drivers every hole; each man sits out
   every third and plays the phantom's ball (1st and 4th shots).
9. **A short team owes four men's worth**, not three — it fields a phantom, and the
   phantom's share rotates through the three real men.
...
A three-man team fields a **phantom 4th**: a fourth slot handicapped at the **average of the
three real men**, whose ball is played by whoever is not driving.
...
- **Nobody is ever borrowed.** A golfer from another team would be hitting shots for a
  rival, and every good one costs his own team the pot. The option does not exist.
...
three real men. Register a `team_play_average` algorithm rather than bending
...
**The TD assigns manually.** No draft, no auto-balance, no random draw — he already knows
who wants to play with whom.

- **Allowance appears per team the moment a fourth man lands**, with the raw sum under the
...
- **Each member row carries his contribution** — `Maiolini 4 → 1.00`, with his percentage
  badge. Showing only the team total hides why moving one man swings a team by two strokes.
...
- **The three-man team carries the phantom row**, italic and undraggable, sorted into the
...
- **Every control shows dollars as it moves, including a per-man figure.** A TD setting
...
- **Ties are warned about here**, where it is cheap: *three places is three prizes, not
  three teams paid.*
- **Per-man figures assume four**, with a line saying the three-man team divides three ways
  and pays more each.
...
to get a scramble card wrong, and it is tapped by a man on the next tee holding a beer.
...
Two men's cards do nothing on a given hole; a man who shot 5 must see instantly that it was
...
UP`). For a three-man team the rota also names who covers the phantom
...
- **Prizes are earned by teams and settled to people.** Every paid place expands to its
  members with each man's figure. `Fern — $287.50` is unactionable at the scoring table.
...
- **Odd cents to the team's highest course handicap.** $287.50 over four is $71.875; split
  ...
  $71.87s.
- **A three-man team splits three ways and takes more each** — $47.91 against Slate's $35.93
...
- **The receipt and the texting flow are the Individual ones, reused unchanged.** A team
  receipt is a short one: one entry line, one prize line if he cashed.
