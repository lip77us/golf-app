...
class TeamDriveStep extends StatelessWidget {
  ...
  /// Best ball and Chapman: both men drive every hole, so there is nothing to
  ...
  /// A team owes its SIZE's worth — four men's worth for a foursome whether it
  /// fields four or three, two men's worth for a pair.
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        // Both men drive every hole with no choice to record. Nothing is
        // disabled without saying why, so the screen states the reason rather
        // than showing four rules that cannot apply.
        if (_hasNoControl)
          const TeamNote(
            'Nothing to set. Both men drive every hole, so there is no drive '
              ...
              children: [
                ...
                TeamNote(
                  _isPairs
                      ...
                      : 'Three men run AB / BC / AC. Two drivers every hole; '
                        "the man sitting out plays the phantom's ball.",
                ),
              ],
            ),
          ),
