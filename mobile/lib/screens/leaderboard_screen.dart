...
class _LeaderboardScreenState extends State<LeaderboardScreen>
  ...
  void _initTabs(Leaderboard lb) {
    ...
    // Triple Cup is a single tab: the per-group match cards, topped by the
    // cross-group cup total + clinch bar.  (A separate "Overview" tab used to
    // duplicate the cup score in bulkier pill cards — folded in here instead.)
  ...
  /// Foursome whose card the rotate-to-landscape gesture shows on the
  /// leaderboard.  Single-foursome rounds resolve directly; multi-group rounds
  /// prefer the viewer's own group, else the first.  Null (→ rotation is a
  /// no-op) when the round isn't loaded here — each group's card is still
  /// reachable by rotating on its game screen.
  int? _landscapeFoursomeId(RoundProvider rp) {
    if (rp.round?.id != widget.roundId) return null;
    final foursomes = rp.round!.foursomes;
    if (foursomes.isEmpty) return null;
    if (foursomes.length == 1) return foursomes.first.id;
    final myPid = context.read<AuthProvider>().player?.id;
    if (myPid != null) {
      for (final fs in foursomes) {
        if (fs.memberships.any((m) => m.player.id == myPid)) return fs.id;
      }
    }
    return foursomes.first.id;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return RoundLandscapeScorecard(
      foursomeId: _landscapeFoursomeId(rp),
      child: Scaffold(
      appBar: GolfAppBar(
        title: 'Leaderboard',
...
class _LowNetViewState extends State<_LowNetView> {
  /// Player rows currently expanded to show their hole-by-hole strip. Multiple
  /// can be open at once; a row toggles closed only when re-tapped. Resets to
  ...
  Widget build(BuildContext context) {
    ...
    results.sort((a, b) {
      ...
      final rb = (b as Map)['rank'] as int? ?? 1 << 30;
    ...
    final anyPayout = results.any((r) => (r as Map)['payout'] != null);

    // Ranked net-to-par table.  Tapping a row expands that player's own
    // hole-by-hole strip inline (accordion); the full stacked grid lives in
    // the rotate-to-landscape scorecard, so this table scales to a big field.
    return ListView(
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            Flexible(
              child: Text(
                "Rotate your phone to see the whole field's scorecard.",
                style: theme.textTheme.bodySmall
                    ?.copyWith(color: theme.colorScheme.onSurfaceVariant),
              ),
            ),
        ...
        const SizedBox(height: 4),
        Text(
          'Tap a player for their hole-by-hole scores.',
  ...
  /// Gross vs par over the holes played so far (in-progress friendly — no raw
  /// total, which isn't meaningful until the round is done).
  int _grossToPar(Map<String, dynamic> row) {
    var d = 0;
    for (final h in (row['holes'] as List? ?? [])) {
      final m = h as Map;
      final g = m['gross'] as int?;
      final p = m['par'] as int?;
      if (g != null && p != null) d += g - p;
    }
    return d;
  }

  List<Widget> _playerBlock(ThemeData theme, Map<String, dynamic> row,
      {required bool showNet, required bool showMoney}) {
    ...
    final holes    = (row['holes'] as List? ?? []);
    final thru     = row['holes_played'] as int? ?? holes.length;
    ...
    final rowWidget = InkWell(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          // Net-to-par first, Gross-to-par second (gross-only rounds show one).
          if (showNet) toParCell(ntp),
          toParCell(grossTp),
          SizedBox(
            width: _colW,
  ...
  /// One player's hole-by-hole strip, styled like the landscape scorecard: a
  /// hole-number header row, a par row, then the player's scores (coloured
  /// circle/square marks + stroke dots).  Subtotals appear only once a segment
  /// is complete — Out after the front 9, In after the back 9, Tot/Net when the
...
class _ByGroupView extends StatelessWidget {
  ...
  /// Optional banner rendered above the group cards (e.g. the Triple Cup
  /// cross-group cup total + clinch bar).
  final Widget? header;

  const _ByGroupView({required this.data, required this.builder, this.header});
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      padding: const EdgeInsets.all(12),
      itemCount: groups.length + (hasHeader ? 1 : 0),
      itemBuilder: (_, i) {
        if (hasHeader && i == 0) {
          return Padding(
        ...
        final gi = hasHeader ? i - 1 : i;
        final g = {
          ...(groups[gi] as Map<String, dynamic>),
...
class _MatchLeaderboardCard extends StatefulWidget {
  ...
  /// shared HoleGridScorecard grid — same as the full Nassau card.
    ...
    return Card(
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (scHoles.isNotEmpty) ...[
            const Divider(height: 20),
            HoleGridScorecard(holes: scHoles, participants: scPlayers,
  ...
  static Iterable<Widget> _pressRows(
    ...
    return presses.map((p) {
      ...
      return Padding(
        ...
        child: Row(children: [
          Text(
            '$labelPrefix${p.pressType == 'manual' ? 'Manual' : 'Auto'} Press holes '
...
class _SixesGroupCard extends StatelessWidget {
  ...
  /// True when this match's pairing has NOT been revealed to the group yet.
  ///
  /// Sixes assigns all three pairings at setup, but the group only LEARNS
  /// matches 2 and 3 when Segment 1 finishes and they spin the wheel (see
  /// the draw in score_entry_screen.dart). Printing them here spoils the
  /// draw, and — worse — reads as settled fact to four golfers who haven't
  /// picked partners yet. An extra match is undrawn until its teams are
  /// actually chosen by the loser of the previous one.
  static bool _isUndrawn(Map<String, dynamic> seg, List segments) {
    List names(String team) {
      final t = seg[team] as Map<String, dynamic>? ?? const {};
      return t['players'] as List? ?? const [];
    }
    if (names('team1').isEmpty || names('team2').isEmpty) return true;

    final standard = segments
        .cast<Map<String, dynamic>>()
        .where((x) => x['is_extra'] != true)
        .toList();
    if (seg['is_extra'] == true) return false;   // teams exist ⇒ chosen
    if (standard.isEmpty || identical(standard.first, seg)) return false;

    final firstStatus = standard.first['status']?.toString();
    return firstStatus != 'complete' && firstStatus != 'halved';
  }
    ...
    return Card(
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (holes.isNotEmpty) ...[
            const Divider(height: 20),
            HoleGridScorecard(holes: holes, participants: players, legend: null,
...
/// Fetches a tournament's cup standings and renders the [_BandonCupScoreboard]
/// (cup-name banner, big team blocks, notched to-win bar) — the same data the
/// tournament card shows.  Tops the Triple Cup leaderboard tab.  Renders nothing
/// until it loads, on failure, or when no points are on the line, so it never
/// leaves a gap.
class _TripleCupScoreboardLoader extends StatefulWidget {
  final int    tournamentId;
  final String cupName;
  const _TripleCupScoreboardLoader({
    required this.tournamentId,
    required this.cupName,
  });

  @override
  State<_TripleCupScoreboardLoader> createState() =>
      _TripleCupScoreboardLoaderState();
}

class _TripleCupScoreboardLoaderState
    extends State<_TripleCupScoreboardLoader> {
  ...
  Future<void> _load() async {
    ...
    } catch (_) {/* leave the tab header empty on failure */}
  }
  ...
  Widget build(BuildContext context) {
    ...
    return _BandonCupScoreboard(
      t1Name       : d['team1_name'] as String? ?? 'Team 1',
      t2Name       : d['team2_name'] as String? ?? 'Team 2',
      t1Colour     : _cupTeamColor(d['team1_colour'] as String?),
      t2Colour     : _cupTeamColor(d['team2_colour'] as String?),
...
class _BandonCupLiveCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Full names for the team header (Triple Cup names each team once in full
    // up top, then uses the short labels in the per-segment rows); falls back
    // to the shorts when the backend didn't send them.
    final t1PlayersFull =
        (match['team1_players_full'] as List? ?? t1Players).cast<String>();
    final t2PlayersFull =
        (match['team2_players_full'] as List? ?? t2Players).cast<String>();
        ...
        child: Column(
          ...
          children: [
            ...
            if (gameType == 'irish_rumble') ...[
              ...
              Builder(builder: (ctx) {
                final leftIsT1   = true /* team 1 always on the left, matching score entry order */;
                // Triple Cup names each team in full here (once); quota_nassau
                // falls back to shorts since it sends no full-name list.
                final leftPly    = leftIsT1 ? t1PlayersFull : t2PlayersFull;
                final leftColor  = leftIsT1 ? t1Colour  : t2Colour;
                final rightPly   = leftIsT1 ? t2PlayersFull : t1PlayersFull;
                final rightColor = leftIsT1 ? t2Colour  : t1Colour;
                // Full two-player names are long, so let each side wrap to two
                // lines rather than truncate — the team is named once here.
                return Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                  Expanded(child: Padding(
...
class _TripleCupSubMatchRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Short labels here — the team is named in full once in the card header;
    // these per-segment rows stay compact.
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        // Golfer initials in their team colors, "vs" in grey.
        Expanded(
          child: RichText(
            overflow: TextOverflow.ellipsis,
            text: TextSpan(
