...
def _gross_to_par(summary, player_id):
    """The reader's own gross against par — the number a golfer checks without
    meaning to, and the one personal figure a neutral board can afford."""
    if player_id is None:
        return None
    total = 0
    played = 0
    for hole in (summary.get('holes') or []):
        par = hole.get('par')
        for e in (hole.get('players') or hole.get('entries') or []):
